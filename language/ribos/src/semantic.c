...
typedef struct RibosHelperParameter {
    const char *name;
    const char *type;
} RibosHelperParameter;

typedef struct RibosHelperSpec {
    const char *path;
    uint32_t capabilities;
    const char *result;
    const char *error;
    RibosHelperParameter parameters[4];
    size_t parameter_count;
} RibosHelperSpec;

static const char *const ribos_predeclared_types[] = {
    "BootContext", "BootAction", "BootError", "BoardRevision",
    "Profile", "Device", "Slot", "Channel", "Core", "Image",
    "VerifiedImage", "RecoveryReason", "Diagnostic", "HandoffKey",
    "ImageId", "UpdateReceipt", "Capability", "DeviceError",
    "NetworkError", "UpdateError", "VerifyError", "BoardFacts",
    "PowerFacts", "SystemFacts", "DeviceSet", "Manifest",
};

static const RibosHelperSpec ribos_helpers[] = {
    {
        "device.init", RIBOS_CAPABILITY_DEVICE, "Result", "BootError",
        {{"device", "Device"}, {"profile", "Profile"}}, 2,
    },
    {
        "slot.selected", RIBOS_CAPABILITY_INSPECT, "Slot", NULL,
        {{NULL, NULL}}, 0,
    },
    {
        "slot.active", RIBOS_CAPABILITY_INSPECT, "Slot", NULL,
        {{NULL, NULL}}, 0,
    },
    {
        "slot.inactive", RIBOS_CAPABILITY_INSPECT, "Slot", NULL,
        {{NULL, NULL}}, 0,
    },
    {
        "slot.failures", RIBOS_CAPABILITY_INSPECT, "u32", NULL,
        {{"slot", "Slot"}}, 1,
    },
    {
        "slot.previous_good", RIBOS_CAPABILITY_STATE, "ResultSlot", "BootError",
        {{NULL, NULL}}, 0,
    },
    {
        "slot.mark_bad", RIBOS_CAPABILITY_STATE, "Result", "BootError",
        {{"slot", "Slot"}}, 1,
    },
    {
        "slot.image", RIBOS_CAPABILITY_INSPECT, "ResultImage", "BootError",
        {{"slot", "Slot"}}, 1,
    },
    {
        "slot.mark_trial", RIBOS_CAPABILITY_STATE, "Result", "BootError",
        {
    ...
    {
        "image.require", RIBOS_CAPABILITY_INSPECT, "ResultImage", "BootError",
        {{"id", "ImageId"}}, 1,
    },
    {
        "image.verify", RIBOS_CAPABILITY_INSPECT, "ResultVerifiedImage",
        "BootError", {{"image", "Image"}}, 1,
    },
    {
        "core.start", RIBOS_CAPABILITY_DEVICE, "Result", "BootError",
        {{"core", "Core"}, {"image", "VerifiedImage"}}, 2,
    },
    {
        "handoff.set", RIBOS_CAPABILITY_HANDOFF, "Result", "BootError",
        {{"key", "HandoffKey"}, {"value", "*"}}, 2,
    },
    {
        "ota.available", RIBOS_CAPABILITY_NETWORK, "bool", NULL,
        {{"channel", "Channel"}}, 1,
    },
    {
        "ota.install",
        RIBOS_CAPABILITY_NETWORK | RIBOS_CAPABILITY_FLASH,
        "ResultUpdateReceipt", "BootError",
        {{"channel", "Channel"}, {"slot", "Slot"}}, 2,
    },
    {
        "network.ready", RIBOS_CAPABILITY_NETWORK, "bool", NULL,
        {{NULL, NULL}}, 0,
    },
    {
        "network.fetch_signed_manifest", RIBOS_CAPABILITY_NETWORK,
        "ResultManifest", "BootError", {{"channel", "Channel"}}, 1,
    },
    {
        "power.safe", RIBOS_CAPABILITY_INSPECT, "bool", NULL,
        {{NULL, NULL}}, 0,
    },
    {
        "diagnostic.note", RIBOS_CAPABILITY_DIAGNOSTIC, "Result", "BootError",
        {{"message", "*"}}, 1,
    },
    {
        "diagnostic.record_verify_error", RIBOS_CAPABILITY_DIAGNOSTIC,
        "Result", "BootError", {{"error", "*"}}, 1,
    },
    {
        "boot.slot", RIBOS_CAPABILITY_BOOT, "BootAction", NULL,
        {{"slot", "Slot"}, {"image", "VerifiedImage"}}, 2,
    },
    {
        "boot.recovery", RIBOS_CAPABILITY_BOOT, "BootAction", NULL,
    ...
    Token *field)
{
    ...
    if (type->name != NULL) {
        if (type->name_length == strlen("BootContext") &&
            memcmp(type->name, "BootContext", type->name_length) == 0) {
            if (ribos_token_equals(field, "board")) {
                return ribos_find_type_name(context, "BoardFacts");
            }
            if (ribos_token_equals(field, "power")) {
                return ribos_find_type_name(context, "PowerFacts");
            }
            if (ribos_token_equals(field, "system")) {
                return ribos_find_type_name(context, "SystemFacts");
            }
            if (ribos_token_equals(field, "devices")) {
                return ribos_add_type(
                    context,
                    RIBOS_TYPE_LIST,
                    NULL,
                    0,
                    ribos_find_type_name(context, "Device"),
                    0,
                    32,
                    0,
                    NULL);
            }
            if (ribos_token_equals(field, "recovery_requested")) {
                return ribos_find_type_name(context, "bool");
            }
        }
        if (type->name_length == strlen("BoardFacts") &&
            memcmp(type->name, "BoardFacts", type->name_length) == 0 &&
            ribos_token_equals(field, "revision")) {
            return ribos_find_type_name(context, "BoardRevision");
        }
        if (type->name_length == strlen("PowerFacts") &&
            memcmp(type->name, "PowerFacts", type->name_length) == 0 &&
            ribos_token_equals(field, "battery_percent")) {
            return ribos_find_type_name(context, "u8");
        }
        if (type->name_length == strlen("SystemFacts") &&
            memcmp(type->name, "SystemFacts", type->name_length) == 0 &&
            ribos_token_equals(field, "on_ground")) {
            return ribos_find_type_name(context, "bool");
        }
    }
    return 0;
...
static const RibosHelperSpec *
ribos_find_helper(const RibosAstNode *path)
{
    size_t index;

    for (index = 0;
         index < sizeof(ribos_helpers) / sizeof(ribos_helpers[0]);
         ++index) {
        if (ribos_path_equals(path, ribos_helpers[index].path)) {
            return &ribos_helpers[index];
        }
    }
    return NULL;
}
...
ribos_helper_result_type(
    RibosSemanticContext *context,
    const RibosHelperSpec *helper)
{
    uint32_t unit = ribos_find_type_name(context, "Unit");
    uint32_t error = helper->error == NULL ?
        0 : ribos_find_type_name(context, helper->error);
    uint32_t value;

    if (strcmp(helper->result, "Result") == 0) {
        value = unit;
    } else if (strcmp(helper->result, "ResultSlot") == 0) {
        value = ribos_find_type_name(context, "Slot");
    } else if (strcmp(helper->result, "ResultImage") == 0) {
        value = ribos_find_type_name(context, "Image");
    } else if (strcmp(helper->result, "ResultVerifiedImage") == 0) {
        value = ribos_find_type_name(context, "VerifiedImage");
    } else if (strcmp(helper->result, "ResultUpdateReceipt") == 0) {
        value = ribos_find_type_name(context, "UpdateReceipt");
    } else if (strcmp(helper->result, "ResultManifest") == 0) {
        value = ribos_find_type_name(context, "Manifest");
    } else {
        return ribos_find_type_name(context, helper->result);
    }
    return ribos_add_type(
        context,
        RIBOS_TYPE_RESULT,
    ...
    RibosStaticCost *cost)
{
    ...
    for (index = 0; index < helper->parameter_count; ++index) {
        ...
        if (strcmp(helper->parameters[index].type, "*") != 0) {
            ...
            if (key_name != NULL &&
                ribos_token_equals(key_name, "BOARD_REVISION")) {
                expected = ribos_find_type_name(
                    context,
                    "BoardRevision");
            } else if (key_name != NULL &&
                ribos_token_equals(key_name, "DELOS_READY")) {
                expected = ribos_find_type_name(context, "bool");
            } else if (key_name != NULL &&
                ribos_token_equals(key_name, "BOOT_SLOT")) {
                expected = ribos_find_type_name(context, "Slot");
            } else {
                (void)ribos_semantic_fail(
                    context,
                    RIBOS_COMPILE_TYPE_ERROR,
    ...
    RibosStaticCost *cost)
{
    ...
    if (callee != NULL && callee->kind == RIBOS_AST_PATH) {
        const RibosHelperSpec *helper = ribos_find_helper(callee);
        RibosFunctionInfo *function;
        size_t function_index;
        uint32_t type_id;
        ...
        if (ribos_path_count(callee) == 1 &&
            ...
            if (argument == NULL || node->items->size != 1) {
                (void)ribos_semantic_fail(
                    ...
                    node,
                    callee->token,
                    "Ok constructor requires one value",
                    NULL,
                    NULL);
                return 0;
            }
            ...
            *cost = ribos_cost_add(*cost, argument_cost);
            if (!ribos_type_compatible(
                ...
                (void)ribos_semantic_fail(
                    ...
                    argument,
                    NULL,
                    "Ok payload does not match the expected Result",
                    NULL,
                    NULL);
                return 0;
            }
        ...
        if (type_id != 0 && ribos_path_count(callee) == 1) {
...
ribos_initialize_types(RibosSemanticContext *context)
{
    ...
    for (index = 0;
         index < sizeof(ribos_predeclared_types) /
             sizeof(ribos_predeclared_types[0]);
         ++index) {
        (void)ribos_add_type(
            context,
            RIBOS_TYPE_NAMED,
            ribos_predeclared_types[index],
            strlen(ribos_predeclared_types[index]),
            0,
            0,
            0,
            0,
