...
`schema/`는 compiler, Policy IR validator와 artifact verifier가 공유하는
versioned product identity 계약이다.

Schema는 다음 table을 canonical stable-ID 순서로 인코딩한다.

- product-generated named type와 copy/affine/linear ownership
- typed fact/member
- helper path, parameter/result, error type, capability와 borrow/consume mode
- typestate transition과 terminal boot-action marker
- policy context/action/error entry ABI
- typed handoff field
...
Schema format 1.1은 위 ownership과 policy ABI를 canonical digest에 포함한다.
`ribon.generic.reference.v1`은 host corpus용 reference schema다. Product/plugin
