...
static int pe_coff_analyze(
    ...
    segment_capacity = out->segment_capacity;
    *out = (struct RibonLoadedPayload){
    ...
    for (uint32_t index = 0u; index < section_count; ++index) {
        ...
        for (uint32_t previous = 0u; previous < out->segment_count; ++previous) {
            uint64_t previous_end;
            if (!pe_add(
                    out->segments[previous].load_address,
                    out->segments[previous].memory_size,
                    &previous_end) ||
                (segment_base < previous_end &&
                 out->segments[previous].load_address < segment_end)) {
        ...
        *segment = (struct RibonLoadSegment){
        ...
        if (segment_base < out->load_base) {
            out->load_base = segment_base;
            out->runtime_load_base = segment_base;
            out->linked_virtual_base = segment_base;
            out->linked_physical_base = segment_base;
        }
    ...
    if (out->segment_count == 0u || !entry_seen || image_end <= out->load_base) {
    ...
    out->memory_size = image_end - out->load_base;
    out->load_plan_flags =
