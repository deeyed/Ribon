...
static int rph1_reserve_section(
    ...
    if (writer == 0 || writer->bytes == 0 || payload_out == 0 ||
        ...
        return RIBON_PROFILE_HANDOFF_STATUS_BAD_ARGUMENT;
    }
    if (ribon_align_up(
            writer->cursor,
        ...
        return RIBON_PROFILE_HANDOFF_STATUS_OUT_OF_CAPACITY;
    }
    ...
    *payload_out = writer->bytes + payload_offset;
    ...
    return RIBON_PROFILE_HANDOFF_STATUS_OK;
}
...
static int rph1_append_region_list(
    ...
    status = rph1_reserve_section(
        ...
        payload_size,
        &payload);
    if (status != RIBON_PROFILE_HANDOFF_STATUS_OK) {
        return status;
    }
...
static int rph1_append_kernel_layout(
    ...
    if (segment_count > RIBON_PARUS_RPH1_MAX_KERNEL_SEGMENTS ||
        ...
        return RIBON_PROFILE_HANDOFF_STATUS_INVALID_PLAN;
    }
...
static int rph1_append_provenance(
    ...
    rph1_write_u32(payload, 0u, (uint32_t)plan->firmware);
...
int ribon_parus_build_rph1(
    ...
    if (status != RIBON_PROFILE_HANDOFF_STATUS_OK || writer.cursor > UINT32_MAX) {
        return status != RIBON_PROFILE_HANDOFF_STATUS_OK ?
                   status :
                   RIBON_PROFILE_HANDOFF_STATUS_OUT_OF_CAPACITY;
    }
